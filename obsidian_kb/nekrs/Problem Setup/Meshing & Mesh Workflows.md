> Source: `source/problem_setup/meshing.rst` (converted to markdown; retains documentation detail)
...
- mesh_setup to load and modify meshes, boundary tags and connectivity
- Generate simple meshes with meshing_nek5000_tools
- meshing_convert from Gmsh, Exodus II, and CGNS

Related topics:

- mesh_moving setup
- meshing_cht meshes
- On-the-fly meshing_hrefine
- Additional meshing tips
- Overview of `mesh_t`

## Runtime Setup

Meshes are stored in the binary re2 file, which contains the
...
modify the mesh when starting from a restart file (see initial_conditions
...
using common utilities such as linalg or a custom kernel.
...
- A **three-character string** that populates the *Nek5000* array
  ...
  generated by tools such as meshing_tool_genbox) and is also
...
Not all meshes carry both tags. in many cases, only one of them is present.
   ...
   "*Gmsh*, *ICEM*, *Cubit* (via mesh convertors)", "Numeric IDs"
...
numeric IDs ``boundaryID(f,e)`` (or ``boundaryIDt(f,e)`` for CHT) to
...
*NekRS* examples. See nek5000_tools for acquiring the tools.
...
Mesh conversion is handled by *Nek5000* tools. See nek5000_tools and the
...
with the fluid-domain mesh, which is useful for setting up meshing_cht
cases.
...
elements to hexahedra. See meshing_tet_to_hex for details. The tool can
...
condition setup, see mesh_setup_sidesets.
...
As illustrated in fig-walls of intro_comput_approach, you must
...
We also recommend following the tutorial conjugate_heat_transfer as a
starting point.
...
below for :math:`N_{\text{cut}} = 3`. See fig-hrefine_mesh for a 2D
illustration.
...
supported. See tab-hrefine_ex1 for examples.
