> Source: `source/problem_setup/case.rst` (converted to markdown; retains documentation detail)
...
See also file_extensions for summarized tables of all *NekRS* files and their extensions.
...
* **User-defined okl file**, with ``.oudf`` extension. This file conventionally has all the user defined device kernels. It must be included in okl_block of ``.udf`` file.
...
* **Session file**, with ``.sess`` extension. This file is for NekNek only.
  (See overlapping_overset_grids tutorial).
...
Optionally, the user can also define the names of ``.udf``, ``.oudf`` and ``.usr`` in the sec:generalpars section and name of ``.re2`` file in sec:meshpars section of ``.par`` file.
...
* ``GENERAL``: generic settings for the simulation (mandatory, see General Parameters<sec:generalpars>)
* ``OCCA``: backend OCCA device settings (optional, see OCCA section<sec:occa>)
* ``PROBLEMTYPE``: settings for the governing equations (optional, see Problem Type<sec:problemtype>)
* ``MESH``: mesh related settings (optional, see Mesh Parameters<sec:meshpars>)
* ``GEOM``: **TODO**
* Field Settings (see Field Settings<sec:field_settings>)
...
.. _tab:generalparams:
   ...
   ``oudf``,"``""<string>""``","Optional name of user-defined OCCA kernel function file |br| As a default *NekRS* expects these are defined in OKL block in ``.udf`` file"
   ``usr``,"``""<string>""``","Optional name of user-defined legacy *Nek5000* (fortran) function file |br| Default is ``<case>.usr``"
   ``regularization``,"","Specifies regularization options for all fields |br| See common field settings<sec:common_settings> for details"
...
.. _tab:meshparams:
   ...
   ``boundaryIDMap``,"``<int>, <int>, ...``", "Map mesh boundary ids to 1,2,3,... |br| See boundary conditions<boundary_conditions> for details"
   ``boundaryIDMapFluid``,"``<int>, <int>, ...``", "Required for conjugate heat transfer cases |br| See boundary conditions<boundary_conditions> for details"
   ...
   ``hrefine``,"``<int>, ...``", "Number of splits per direction along an element |br| See meshing_hrefine for details."
...
.. _tab:scalarparams:
   ...
   ``mesh``,``fluid`` |br| ``+ solid``, "Specifies the mesh region where scalar ``FOO`` is solved (relevant to CHT case) |br| Default = ``fluid``"
...
.. _tab:commonparams:
   ...
   ``solver``,"``none`` |br| ``user`` |br| ``cvode`` |br| ``CG`` |br| ``+ combined`` |br| ``+ block`` |br| ``+ flexible`` |br| ``+ maxiter=<int>`` |br| ``GMRES`` |br| ``+ flexible`` |br| ``+ maxiter=<int>`` |br| ``+ nVector=<int>`` |br|  ``+ iR``","Solve off |br| user-specified |br| CVODE solver (see sec:cvodepars) |br| Conjugate gradient solver. **Default solver for velocity and scalar equation** |br| **Default for scalar equation** |br| **Default velocity solver** |br| . |br| . |br| . |br| Generalized Minimal Residual solver. **Default solver for pressure** |br| **Default for pressure** |br| . |br| Dimension of Krylov space |br| Iterative refinment"
   ...
   ``boundaryTypeMap``,"``<bcType for ID 1>, <bcType for ID 2>, ...``","See boundary_conditions for details"
...
Details of the ``udfDirichlet`` and ``udfNeumann`` functions used for Dirichlet and Neumann boundary conditions, respectively, can be found in boundary_conditions.
...
In this example, ``UDF_Setup0`` reads the ``p_gamma`` key from the ``CASEDATA`` user section in the ``.par`` file (see sec:user_section) using the convenience helper ``platform->par->extract``.
...
* Assign initial conditions (see initial_conditions).
* Mesh manipulation (see tutorial_rans tutorial).
* Register function pointers for user-defined spatially varying material properties (see properties).
* Register function pointers for user-defined source terms (see source_terms).
* Initialize and set up RANS turbulence models (see ktau_model)
* Initialize and set up the Low-Mach compressible model (see lowmach_model)
* Initialize solution recycling routines and arrays (see boundary_conditions_recycling)
* Allocate ``bc->o_usrwrk`` for user-defined boundary data (see *TODO*)
* Initialize time-averaging routines and buffers (see postproc_averaging_tavg)
...
* Run time-averaging updates (see postproc_averaging_tavg)
* Invoke solution recycling logic (see boundary_conditions_recycling).
* Post-processing tasks, such as:

  * Extracting data over a line (see extract_line).
  * Writing custom field files (see postproc_checkpoint_custom_file).
...
See meshing for mesh creation and conversion workflows.

You can obtain ``.re2`` by:

* Porting from an existing *Nek5000* case,
* Using a mesh converter<meshing_convert> from other format, or
...
Before launching, set a catchable signal in ``NEKRS_SIGNUM_UPD`` (see nekrs_signal).
...
## NekRS Files & Extensions
   ...
   ".sess","NekNek sessions file","Yes (Only for NekNek)","See sess_file"
   ".par","Main parameter file","Yes","See par_file"
   ".udf","User-defined functions","Yes","See udf_file"
   ".oudf","Extra OKL kernels file","No","See okl_block"
   ".upd","Trigger file","No","See upd_file"

   ".usr","Legacy Nek5000 user file","No","See usr_file"
   ".re2","Mesh file","Yes","See re2_file"
   ...
   "\*0.f0000","Nek format checkpoint file","0-based index. See qstart_vis"
   ".nek5000","Nek format checkpoint metadata file", "See qstart_vis"
   ".bp/","ADIOS2 format checkpoint (folder)","See postproc_adios"
